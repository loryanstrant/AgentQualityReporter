"""Symmetric encryption for secrets at rest (Dataverse SP secret, Foundry key).
...
Any non-empty ``FERNET_KEY`` is accepted: a value that is already a valid Fernet
key is used verbatim, otherwise a stable key is derived from it (SHA-256 →
urlsafe-base64), so one-touch deployments that auto-generate a GUID still work.
"""
